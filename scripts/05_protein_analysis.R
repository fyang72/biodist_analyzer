...
ui_protein_analysis <- function() { 
  
#tabPanel(title="protein", id="panel_protein",   
         #tabPanel(title="Heatmap", id="tab_page1",

tagList(    
         fluidRow(
           column(width = 6, box(title = "Primary-heatmap", width = NULL, solidHeader = TRUE, status = "primary", originalHeatmapOutput("ht_protein", width = 600, height = 350, containment = TRUE))),
           column(width = 6, box(title = "Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", subHeatmapOutput("ht_protein", title = NULL, width = 600, height = 350, containment = TRUE))),
           column(width = 12, htmlOutput("note_protein"))
         ),
         tabsetPanel(
           tabPanel("Table",
                    fluidRow(
                      column(width = 12, box(title = "Result Table in Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", DT::dataTableOutput("resulting_table_protein", width = "100%", height = "auto", fill = TRUE)))
                    )
           ),
           tabPanel("Boxplot",
                    fluidRow(
                      column(width = 12, box(title = "Boxplot of Analyte(s) vs Timepoint(s) in Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", plotOutput("boxplot_protein", width = "100%", height = 700)))
                    )
           ),
...
# 
# make_heatmap_protein <- function(df0, env, trt_group, paramcd_f, atpt_f, sex_f, aval_range = c(0, 9999), add_aval_txt = FALSE) {
...
#   env$protein$tdata_filtered <- df_filtered
#   env$protein$df_filtered_pivoted <- df_filtered_pivoted
#   env$protein$row_index <- seq_len(nrow(df_filtered_pivoted))
#   env$protein$col_index <- seq_len(ncol(df_filtered_pivoted))
#   
#   ht = create_heatmap2(
#     df_filtered_pivoted,
#     df0,
...
outputs_after_brush_action_protein <- function(res,input, output, session, env) {
  
  output[["boxplot_protein"]] <- renderPlot({
    
   df = env$protein$tdata_filtered %>%
      filter(USUBJID %in% rownames(res), MATRIXCD %in% colnames(res))
   
   make_boxplot_protein(df)
    
  }, height = 700)
  
  output[["resulting_table_protein"]] <- DT::renderDataTable(
  ...
  output[["note_protein"]] <- renderUI({
    ...
    note <- spec4matrix %>%
      filter(MATRIXCD %in% (env$protein$tdata_filtered %>% pull(MATRIXCD) %>% unique())) %>%
      arrange(MATRIXCD) %>%
...
make_boxplot_protein <- function(df) {
