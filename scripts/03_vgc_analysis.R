...
tagList(    
         fluidRow(
           column(width = 6, box(title = "Primary-heatmap", width = NULL, solidHeader = TRUE, status = "primary", originalHeatmapOutput("ht_vgc", width = 600, height = 350, containment = TRUE))),
           column(width = 6, box(title = "Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", subHeatmapOutput("ht_vgc", title = NULL, width = 600, height = 350, containment = TRUE))),
           column(width = 12, htmlOutput("note_vgc"))
         ),
         tabsetPanel(
           tabPanel("Table",
                    fluidRow(
                      column(width = 12, box(title = "Result Table in Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", DT::dataTableOutput("resulting_table_vgc", width = "100%", height = "auto", fill = TRUE)))
                    )
           ),
           tabPanel("Boxplot",
                    fluidRow(
                      column(width = 12, box(title = "Boxplot of Analyte(s) vs Timepoint(s) in Sub-heatmap", width = NULL, solidHeader = TRUE, status = "primary", plotOutput("boxplot_vgc", width = "100%", height = 700)))
                    )
           ),
...
# 
# make_heatmap_vgc <- function(df0, env, trt_group, paramcd_f, atpt_f, sex_f, aval_range = c(0, 9999), add_aval_txt = FALSE) {
...
#   env$vgc$tdata_filtered <- df_filtered
#   env$vgc$df_filtered_pivoted <- df_filtered_pivoted
#   env$vgc$row_index <- seq_len(nrow(df_filtered_pivoted))
#   env$vgc$col_index <- seq_len(ncol(df_filtered_pivoted))
#   
#   ht = create_heatmap2(
#     df_filtered_pivoted,
#     df0,
