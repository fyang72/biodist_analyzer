...
make_heatmap = function(tdata0, trt_group, paramcd, atpt, sex, aval_range=c(0, 9999), 
  ...
  tdata_filtered <-  tdata0 %>% 
    ...
    dplyr::filter(
      ATPT_f  %in% "Week 26/27",           # "Week 6"     "Week 26/27" "Pretest"
      PARAMCD  %in% "vgc"
    ) %>% 
  ...
  meta4_subj <- tdata_filtered %>% as.data.frame() %>% distinct(USUBJID, GROUP, ARM_f) %>% arrange(GROUP, USUBJID)
